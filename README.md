...
视觉上像 ChatGPT 自带的网络状态组件，但技术上完全独立（Shadow DOM 隔离），可自由拖动、位置自动保存。
...
- **可自由拖动**：按住 HUD 空白区域或左侧 `⋮⋮` 手柄拖动，松开后保留当前位置，并限制在窗口可见范围内。
...
- **拖动**：按住左侧 `⋮⋮` 手柄或卡片空白区域（非 IP / 刷新按钮等控件）自由拖动；松开后保留当前位置，并自动限制在窗口可见范围内。
- **双击复位**：双击手柄恢复默认位置 `top:72px; right:12px`。
- **展开**：悬停约 250ms（可关闭），或点击状态条固定展开；根据 HUD 所在屏幕一侧向内扩展，不会超出视口。
